...
router.post("/:hotelId/bookings", verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    // checking for errors in the payment intent
    if (!paymentIntent) {
      return res.status(400).json({ message: "Payment Intent not found" })
    }

    //check if the hotel id and user id check up, if the reservation is not gonna be done for a wrong hotel or a wrong user 
