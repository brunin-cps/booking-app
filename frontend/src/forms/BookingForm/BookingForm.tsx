...
const BookingForm = ({ currentUser, paymentIntent }: Props) => {
  ...
  const { mutate: bookRoom, isLoading } = useMutation(apiClient.createRoomBooking, {
    ...
    onError: () => {
      showToast({ message: "Booking Failed on Save", type: "ERROR" })
  ...
  // console.log(paymentIntent.totalCoast)

  return (
    <form action="" className="grid grid-cols-1 gap-5 rounded-lg border border-slate-300 p-5" onSubmit={handleSubmit(onSubmit)}>
      <span className="text-3xl font-bold">Confirm Youe Details</span>
      ...
      <div className="space-y-2">
        ...
        <div className="bg-blue-200 p-4 rounded-md">
          <div className="font-semibold text-lg">
            Total Coast: R${paymentIntent.totalCost}
          </div>
