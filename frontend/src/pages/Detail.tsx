...
const Detail = () => {
  ...
  const { data: hotel } = useQuery(
    ...
    { enabled: !!hotelId } // wait for hotelId be available to make the request
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl tonf-bold">{hotel.name}</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr]">
        ...
        <div className="h-fit">
          <GuestInfoForm hotelId={hotel._id} pricePerNight={hotel.pricePerNight}></GuestInfoForm>
        </div>
      </div>
    </div>
  )
