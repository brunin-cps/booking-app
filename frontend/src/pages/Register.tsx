...
const Register = () => {
  ...
  const mutation = useMutation(apiClient.register, {
    onSuccess: async () => {
      showToast({ message: "Registration was sucessfull!", type: "SUCCESS" })
  ...
  return (
    <form className="flex flex-col gap-5" onSubmit={onSubmmit}>
      <h2 className="text-3xl font-cold">Create Account</h2>
      <div className="flex md:flex-row flex-col gap-5">
        <label className="text-gray-700 text-sm font-bold flex-1" >
          ...
          {errors.firstName && (
            <span className="text-red-500 font-normal">{errors.firstName.message}</span>
          )}
        </label>
        ...
        <label className="text-gray-700 text-sm font-bold flex-1">
          ...
          {errors.lastName && (
            <span className="text-red-500 font-normal">{errors.lastName.message}</span>
          )}
        </label>
      </div>
      <label className="text-gray-700 text-sm font-bold flex-1">
      ...
      <label className="text-gray-700 text-sm font-bold flex-1">
        ...
        <input type="password" className="border rounded w-full py-1 px-2 font-normal"{...register("confirmPassword", {
          validate: (val) => {
            ...
            else if (watch("password") !== val) return "Passwords doesnt match"
          }
        })}></input>
        {errors.confirmPassword && (
          <span className="text-red-500 font-normal">{errors.confirmPassword.message}</span>
        )}
      </label>
      <span>
